...
class DigitalGiftPaypalController extends Controller
{
    ...
    public function index()
    {
        ...
        $paginator = BasePaginator::getDefault(
            ...
            function ($params) {
                $builder = ExchangeRequest::ofDigitalGiftPaypal();
                // ID検索
        ...
        return view('digital_gift_paypal_index', ['paginator' => $paginator]);
    }
    ...
    public function resend(int $exchange_request_id)
    {
        ...
        $res = $exchange_request->sendDigitalGiftUrl($url,'PayPal');

        return redirect()->back()
            ->with('message', $res ? 'デジタルギフトPayPalのURLの再送信に成功しました' : 'デジタルギフトPayPalのURLの再送信に失敗しました');

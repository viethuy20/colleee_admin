...
class PointMonthlyReportsDaily extends Command
{
    /**
     ...
     * @var string
     */
    protected $signature = 'point_month:report_daily';
     ...
     * @var string
     */
    protected $description = 'Save point monthly reports daily';
    ...
    public function handle()
    {
        ...
        $firstDayOfThisMonth = $firstDayOfThisMonth->format('Y-m-d 00:00:00');

        // get yesterday
        $yesterdayOfThisMonth = date('Y-m-d 23:59:59', strtotime('-1 days'));
        $lastDayOfLastMonth = $yesterdayOfThisMonth; 

        // if current day is 1
        if ($currentDay == 1) {
            // get first day of last month
            $firstDayOfThisMonth = new \DateTime("first day of last month");
            $firstDayOfThisMonth = $firstDayOfThisMonth->format('Y-m-d 00:00:00');
        ...
        $sql = "DELETE FROM point_monthly_reports WHERE report_day = '$firstDayOfThisMonth'";
        DB::delete($sql);
      ...
      FROM
        (
          (
            SELECT
              DATE_FORMAT('$firstDayOfThisMonth', '%Y-%m-01') AS report_day
            ...
            WHERE
              (id, user_id) IN (
                ...
                FROM
                  user_points
                WHERE
                  created_at <= '$yesterdayOfThisMonth'
                GROUP BY
                  user_id
              )
          )
          UNION (
            ...
            WHERE
              ...
              AND aff_rewards.actioned_at BETWEEN '$firstDayOfThisMonth' AND '$lastDayOfLastMonth'
            GROUP BY
              report_day
            ORDER BY
              report_day
          ...
          UNION (
            ...
            FROM
              user_points
            WHERE
              user_points.created_at BETWEEN '$firstDayOfThisMonth' AND '$lastDayOfLastMonth'
          ...
          UNION (
            ...
            FROM
              exchange_requests
            WHERE
              exchange_requests.created_at BETWEEN '$firstDayOfThisMonth' AND '$lastDayOfLastMonth'

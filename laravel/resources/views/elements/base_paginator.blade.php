...
<ul class="pagination">
    ...
    <li>{{ Tag::link($paginator->url(1), '<<') }}</li>
    <li class="previous">{{ Tag::link($paginator->url($paginator->currentPage() - 1), '<') }}</li>
    @else
    ...
    @else
    <li>{{ Tag::link($paginator->url($i), $i) }}</li>
    @endif
    @endfor
    ...
    <li class="next">{{ Tag::link($paginator->url($paginator->currentPage() + 1), '>') }}</li>
    <li>{{ Tag::link($paginator->url($paginator->lastPage()), '>>') }}</li>
    @else
